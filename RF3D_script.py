...
dem_min=BlockStatistics(dem,NbrRectangle(10,10,"CELL"),"MINIMUM")
dem_min.save(ws+"\dem_min")
arcpy.AddMessage("Minumun Dem ulu�turuluyor")
dem_mean=BlockStatistics(dem,NbrRectangle(10,10,"CELL"),"MEAN")
...
#Soil type dosyası içerisinde sadece soiltype alanı kalmasını sağlıyoruz
st_fields = [c.name for c in arcpy.ListFields(st) if not c.required]
st_silinecekler=[]
for b in st_fields:
    if b=="soiltype":
        pass
    else:
        st_silinecekler.append(b)
arcpy.DeleteField_management(st,st_silinecekler)
#--------------------------------------------------------------------
...
arcpy.AddMessage("BUaraya kadar tamam Kontrol 1")

rec_data=arcpy.ListFiles("st_*.tif")
##Raster lar tabloya dönüştürülüyor
for j in rec_data:
    ad2=str(j)
    ad2.split()
...
##ASC formatında kaydettiğimizde decimal ayracı virgül oluyor bu yüzden TXT kaydedeceğiz
...
##Rocks tablosu alanların hepsi siliniyor nid alanı ekleniyor sadece nid alanının kalması sağlanıyor
...
arcpy.CalculateField_management(rocks,"density",density,"PYTHON")##Kayaçların yoğunluğu
...
##Kayaların yükseklikleri hesaplanıyor
...
arcpy.env.cellSize = cellsize
##Yükseklikler kontrol ediliyor hata olanlara 1 yazacak
...
##disk şekilli bloklar belirleniyor
...
##küre olan bloklar belirleniyor
...
##elips ve dikdörtgen olan bloklar belirleniyor
...
arcpy.AddMessage("CREATING ASCII Files......"+cellsize)
...
##
##if os.path.exists:
##    arcpy.AddMessage("BIR SORUN VAR temp DOSYALARI SİLEMEDİM")
##else:
##    arcpy.AddMessage("GECİCİ DOSYALAR DA SİLİNDİ_ HER ŞEY YOLUNDA")
