...
arcpy.AddMessage("Minumun Dem uluþturuluyor")
dem_mean=BlockStatistics(dem,NbrRectangle(6,6,"CELL"),"MEAN")
dem_mean.save(ws+"\dem_mean")
arcpy.AddMessage("Mean Dem uluþturuluyor")

arcpy.AddMessage("RG dosyasý oluþturuluyor")
roughness=dem_mean-dem_min
roughness.save(ws+"/rg.tif")

arcpy.AddMessage("Sýnýflandýrma dosyasý oluþturuluyor")
...
##Soil tiplerini ayrý ayrý bölüyor
...
arcpy.AddMessage("Zemin tipleri ayrý ayrý kaydediliyor")
...
##Raster lar tabloya dönüþtürülüyor
for j in rec_data:
    ad2=str(j)
    ad2.split()
...
##ASC formatýnda kaydettiðimizde decimal ayracý virgül oluyor bu yüzden TXT kaydedeceðiz
...
##Rocks tablosu alanlarýn hepsi siliniyor nid alaný ekleniyor sadece nid alanýnýn kalmasý saðlanýyor
...
##Kayalarýn yükseklikleri hesaplanýyor
...
##disk þekilli bloklar belirleniyor
...
##
##if os.path.exists:
##    arcpy.AddMessage("BIR SORUN VAR temp DOSYALARI SÝLEMEDÝM")
##else:
##    arcpy.AddMessage("GECÝCÝ DOSYALAR DA SÝLÝNDÝ_ HER ÞEY YOLUNDA")
